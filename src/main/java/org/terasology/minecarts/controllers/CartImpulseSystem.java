...
public class CartImpulseSystem extends BaseComponentSystem {
    ...
    private boolean areJoinedTogether(EntityRef entity, EntityRef otherEntity) {
        ...
        return joint.findJoint(otherEntity) != null;
    }
    ...
    private void handleCharacterCollision(CollideEvent event, EntityRef entity) {
        ...
        Vector3f r1v = new Vector3f(event.getNormal().x / r1.mass, event.getNormal().y / r1.mass,
                event.getNormal().z / r1.mass).mul(lambda);
        Vector3f r2v = new Vector3f(event.getNormal().x / Constants.PLAYER_MASS,
                event.getNormal().y / Constants.PLAYER_MASS, event.getNormal().z / Constants.PLAYER_MASS).mul(lambda).invert();
    ...
    private void handleCartCollision(CollideEvent event, EntityRef entity) {
        ...
        Vector3f df =
                new Vector3f(v2l.getWorldPosition()).sub(v1l.getWorldPosition()).add(new Vector3f(Float.MIN_VALUE,
                        Float.MIN_VALUE, Float.MIN_VALUE)).normalize();
